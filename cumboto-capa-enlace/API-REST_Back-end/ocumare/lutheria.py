...
class tsco(object):
	...
	#	nombre -> nombre de la aplicación en la biblioteca
        ...
        #######################################################################
	@_serializar
        def nv_edo_app_rell(self, codigo_apps):
                # Verificación del codigo de la aplicación según el patrón
                if re.match('[0-9A-Fa-f]{12}', codigo_apps):
                # Verificacion de existencia de la aplicación
                # en el directorio de la Biblioteca de aplicaciones
                        if os.path.isdir(os.path.join(self.app_biblio, codigo_apps)):
				self.red_edo_reg.set('Red', 'app_rell', codigo_apps)
				with open(os.path.join(self.ruta_trans, 'estado_red.conf'), 'w') as f_r:
					self.red_edo_reg.write(f_r)
				return True
                        else:
                                return False
                else:
	...
	def nv_edo_omision(self, s, a):
        ...
        #####################################################################
	@_serializar
        def detener_todo_serv(self,s):
                for i in range(self.ns):
			ruta_conf = '/etc/cumaco/ocumare.conf'
                        self.conf.set('servicio_%s' % i, 'num_app', '1')
			sf = open(ruta_conf, 'w')
			self.conf.write(sf)
			if s in range(len(self.svc_md)):
				self.gen_nvtsco({
					'serv': s,
					'app': 0,
					'cod': self.app_rell,
        ...
        #####################################################################
	@_serializar
	def detener_todo_serv_2(self, s):
		for i in range(self.ns):
			if i == s: 
				na = int(self.conf.get('servicio_%s' % i, 'num_app'), 0)
				for j in range(na):
